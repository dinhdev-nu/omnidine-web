...
export function useMainPosMenuData({
  restaurantId,
  activeCategory,
  searchQuery,
}: UseMainPosMenuDataParams) {
  ...
  const categoriesFetch = useFetch(fetchActiveCategories, categoriesFetchArgs, {
    enabled,
  })
  const itemsFetch = useFetch(fetchMenuItemsByCategory, itemsFetchArgs, {
    enabled,
  })
  ...
  const uiCategories = useMemo(() => {
    ...
    const totalActiveItems = activeCategories.reduce(
      (sum, category) => sum + category.item_count,
      0
    )
