...
import { INITIAL_TABLES, INITIAL_TABLE_POSITIONS } from "../constants"
import {
  clamp,
  getDefaultPosition,
  toTableFromListItem,
...
export const useTableManagement = () => {
  ...
  const [tableOccupancyById, setTableOccupancyById] = useState<
          ...
          setTables((prev) =>
            mapped.map((table) => {
              ...
              return {
                ...
                qr_code: qrCode,
                qr_url: qrCode
                  ? `${window.location.origin}/public/tables/${qrCode}`
                  : null,
              }
            })
          )
        ...
        if (alive) {
          setTables((prev) =>
            prev.map((table) => {
              ...
              const nextNotes =
                "notes" in detail ? detail.notes : (table.notes ?? null)
              return {
                ...table,
          ...
          setTableOccupancyById((prev) => {
            ...
            if (detail.status === "occupied") {
              next[selectedTableId] = clamp(
                prev[selectedTableId] || 1,
                1,
                detail.capacity
              )
            } else {
  ...
  const handleUpdateTable = useCallback(
    (id: string, form: UpdateTablePayload) => {
      ...
      void (async () => {
        ...
        try {
          ...
          setTables((prev) =>
            prev.map((table) => {
              ...
              return {
                ...
                qr_url: response.table.qr_code
                  ? `${window.location.origin}/public/tables/${response.table.qr_code}`
                  : null,
              }
            })
          )
          setTableOccupancyById((prev) => ({
            ...prev,
            [id]:
              response.table.status === "occupied"
    ...
    (id: string) => {
      ...
      void (async () => {
        ...
        try {
          ...
          toast.success(
            result.is_active ? "Đã kích hoạt bàn" : "Đã ngưng hoạt động bàn"
          )
        } catch (error) {
  ...
  const handleRegenerateTableQr = useCallback(
    (id: string) => {
  ...
  const handleAutoArrange = useCallback(() => {
    setTablePositions((prev) => {
      const next = { ...prev }
      const movingTables = tables.filter(
  ...
  const handleStatusFilterChange = useCallback((value: string) => {
    const allowed: TableStatusFilter[] = [
      "all",
      "available",
      "occupied",
      "reserved",
