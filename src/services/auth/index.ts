...
export async function checkEmail(email: string): Promise<CheckEmailResponse> {
  const response = await apiClient.post<ApiSuccessResponse<CheckEmailResponse>>(
    "/auths/check-email",
    { email }
  )
...
export async function verifyEmailOtp(
  ...
  const response = await apiClient.post<
    ApiSuccessResponse<{ verified: boolean }>
  >("/auths/verify-otp", { email, otp })
  return unwrapResponseData(response)
}
...
export async function send2faOtp(
  ...
  const response = await apiClient.post<
    ApiSuccessResponse<{ message: string; expires_in: number }>
  >("/auths/2fa/send-otp", {
...
export async function logout(): Promise<void> {
  await apiClient.post<ApiSuccessResponse<{ logged_out: boolean }>>(
    "/auths/logout"
  )
}
...
export async function verifyResetPasswordOtp(
  ...
  const response = await apiClient.post<
    ApiSuccessResponse<VerifyResetOtpResponse>
  >("/auths/reset-password/verify-otp", {
...
export async function resetPassword(
  ...
  await apiClient.post<ApiSuccessResponse<{ reset: true }>>(
...
export async function changePassword(
  ...
  await apiClient.post<ApiSuccessResponse<{ changed: true }>>(
...
export async function enable2fa(password: string): Promise<{ enabled: true }> {
  const response = await apiClient.post<ApiSuccessResponse<{ enabled: true }>>(
    "/auths/2fa/enable",
    { password }
  )
  return unwrapResponseData(response)
...
export async function disable2fa(
  ...
  const response = await apiClient.post<ApiSuccessResponse<{ disabled: true }>>(
    "/auths/2fa/disable",
    { password }
  )
  return unwrapResponseData(response)
...
export async function getSessions(): Promise<SessionInfo[]> {
  const response =
    await apiClient.get<ApiSuccessResponse<{ sessions: SessionInfo[] }>>(
      "/auths/sessions"
    )
